...
- Bước 1 (Khảo sát / Triage): Hỏi thăm cụ thể vị trí chấn thương/đau mỏi (cổ vai gáy, thắt lưng, gãy tay cũ...), tiền sử điều trị và mức độ cử động hiện tại.
...
- Khi khách hàng chia sẻ số điện thoại, chủ động thu thập để liên hệ đặt lịch tư vấn trực tiếp.`;
...
function extractPhoneAndInfo(text: string) {
  ...
  return {
    fullName,
    phone,
    healthCondition: 'Tư vấn phục hồi & trị liệu qua Vici Care AI',
    serviceInterest: 'Kiểm tra tầm vận động (ROM test) & Tập thử'
  };
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Models priority: user specified -> cost-effective 2.5 / flash-lite models -> standard flash
              ...
              const followUpContents = [
                ...
                { role: 'model' as const, parts: [{ functionCall: call }] },
                {
                  ...
                  parts: [{
                    functionResponse: {
                      name: 'save_contact_lead',
                      response: { status: 'success', message: 'Thông tin đã được ghi nhận.' }
                    }
                  }]
                }
              ];
