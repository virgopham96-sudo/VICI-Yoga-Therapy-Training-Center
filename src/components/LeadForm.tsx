...
export default function LeadForm({ prefilledCourse, onLeadSubmitted }: LeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const newLeadData = {
      ...
      category: primaryInterest.toLowerCase().includes('hlv')
        ? ('TRAINER_EDUCATION' as const)
        : primaryInterest.toLowerCase().includes('trị liệu') || primaryInterest.toLowerCase().includes('scan')
        ? ('THERAPY_INTEREST' as const)
        : primaryInterest.toLowerCase().includes('nâng cao')
        ? ('ADVANCED' as const)
        : ('BEGINNER' as const),
      experience,
      goals: selectedGoals,
      preferredTime: timePreference || 'Linh hoạt',
      recommendedCourse: primaryInterest,
      leadScore: 'HOT' as const,
      staffNotes: notes
        ? `Khóa tích chọn: [${primaryInterest}]. Ghi chú khách: ${notes}`
        : `Khóa tích chọn: [${primaryInterest}] qua Website Form`,
      nextAction: 'Gọi điện xác nhận tư vấn trong 24h',
    };
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 rounded-xl bg-white border border-[#D5C7AA] focus:border-[#D69A2D] focus:ring-1 focus:ring-[#D69A2D] outline-none text-xs sm:text-sm text-[#252822] placeholder:text-gray-400"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 rounded-xl bg-white border border-[#D5C7AA] focus:border-[#D69A2D] outline-none text-xs sm:text-sm text-[#252822] placeholder:text-gray-400"
                    />
                  </div>
                </div>
              </div>
